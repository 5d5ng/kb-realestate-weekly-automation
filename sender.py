...
def send_alimtalk(message: str, phone_numbers: list[str]) -> dict[str, Any]:
    """현재는 알림톡 대신 동일 문안을 SMS로 발송"""
    return send_sms(message=message, phone_numbers=phone_numbers)
...
def send_all(
    ...
    if instagram_caption:
        results["instagram"] = post_instagram(instagram_caption, image_url=image_url)
